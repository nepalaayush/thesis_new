...
To evaluate the accuracy of edge tracking, the following graphs were obtained: 
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{average_cost_plot}
	\caption{Average cost value per point for all datasets for unloaded condition across frames}
	\label{fig:averagecostplot}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{avg_of_avg}
	\caption{Bar diagram showing the average cost value across all frames for each dataset}
	\label{fig:avgofavg}
\end{figure}
...
\section{Discussion}
Given that the subject movement is restricted to only a single plane, it is reasonable to assume that the edges of the bone undergo motions that can be described by rigid transformations.
