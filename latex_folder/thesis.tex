...
The final selection of the desired edge was accomplished using scipy's label algorithm (v1.11.3). The goal was to isolate an edge that accurately represents the overall shape of the bone slice. Labelling was useful in this regard as it allowed for easy separation of distinct edges. Figure \ref{fig:labelimg} shows the output from the labelling algorithm, where each color represents a separate label. 
...
Step 3: Obtaining the set of reference points

Upon successfully isolating the binary edge images of the tibia and femur in Step 2, the subsequent step focused on establishing a set of reference points along each bone's edge. These points are the basis for calculating the transformation between subsequent frames. The first frame, when the knee is in a fully flexed position, was used as the reference frame. The rest of frames remain as binary edges while only the first frame is downsampled into a set of equidistant points along the boundary. To begin, the points along the boundary were organised and sorted. This was done by first, defining a starting point, and then iteratively using a greedy nearest neighbor algorithm for sorting. The most distal point of the bone was taken as the initial seed. Various functions from the numeric python library 'Numpy' (v. 1.26.4) were employed for this task. 


Continuing from the initial sorting of points, the next crucial step involves transforming the sorted sequence of boundary points into a uniformly distributed set of reference points. This is achieved through the application of an equidistant sampling technique designed to accurately represent the geometrical contour of the bone edge with a reduced number of points. Specifically, the goal is to distribute a predefined number of points (e.g., 50 points) along the curve such that the distance between consecutive points is approximately equal.

This process begins by calculating the cumulative distances between each consecutive pair of points in the sorted list. Using these distances, a total path length is established. The desired interval between the new points is then determined by dividing this total path length by the number of intervals (number of points minus one). The algorithm proceeds by placing the first new point at the starting point of the sorted array. Subsequent points are positioned by moving along the path, adding the desired interval to the last placed point until the end of the path is reached. This method ensures that the new points are spaced as uniformly as possible along the entire length of the bone edge, thereby capturing the shape and curvature of the bone with high fidelity.

The selection and interpolation between the original points are conducted using linear interpolation methods, which are efficiently executed using capabilities provided by Numpy. This approach ensures that each new point lies exactly on the linear segment between two existing points, maintaining the anatomical accuracy of the bone representation in the reference frame. 

hen, the sequence of points were processed to select the points that were equidistant from each other. This was accomplished by calculating the cumulative distance along the boundary and dividing the total by the desired number of points minus one. Figure \ref{fig:downsampled} depicts this result. 
\begin{figure}[H]
	\centering
