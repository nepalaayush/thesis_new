...
#%%
first_half_df = pd.read_pickle('ismrm_final/first_half_trans_and_angle.pkl') 
second_half_df = pd.read_pickle('ismrm_final/second_half_trans_and_angle.pkl')
...
fig.savefig('ismrm_final/Figure 4.svg', bbox_inches='tight')
fig.savefig('ismrm_final/Figure 4.png', bbox_inches='tight', dpi=300)
#%%
def plot_four_panel_translation(df_first_half, df_second_half, bin_width=10, figsize=(20, 15), dpi=300, y_padding=0.1, is_y_padding=0.3):
    # Reset matplotlib params to default at the start
    plt.rcParams.update(plt.rcParamsDefault)
    ...
    def plot_data(df, ax, column, title, is_is=False, show_legend=False):
        df_copy = df.copy()
        bin_edges = list(range(0, 101, bin_width))
        df_copy['Custom_Bin'] = pd.cut(df_copy['Percent Flexed'], bins=bin_edges, include_lowest=True)
        df_copy['Bin_Center'] = df_copy['Custom_Bin'].apply(lambda x: x.mid)
    ...
    # Plot the panels
    plot_data(df_first_half, axs[0, 0], 'AP_Translation', 'Anterior (+ve) / Posterior (-ve)', show_legend=True)
    plot_data(df_first_half, axs[0, 1], 'IS_Translation', 'Superior (+ve) / Inferior (-ve)', is_is=True)
    plot_data(df_second_half, axs[1, 0], 'AP_Translation', 'Anterior (+ve) / Posterior (-ve)')
    plot_data(df_second_half, axs[1, 1], 'IS_Translation', 'Superior (+ve) / Inferior (-ve)', is_is=True)
